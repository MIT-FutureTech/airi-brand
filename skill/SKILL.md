...
**Logos:** A single AIRI Initiative logo is used for all projects under the Initiative, including the AI Risk Repository and AI Risk Index. Files live in `logos/initiative/`:

- `icon.*` — square mark only (the "plus of squares" composition)
- `horizontal.*` — icon + "MIT AI Risk / Initiative" wordmark lockup
...
3. Use the appropriate logo variant for the project context. There is only one AIRI logo; do not invent per-project variants.
